...
//            Section {
                    ...
                    Task {
                        ...
                        StorageService.shared.uploadProfileImage(data, for: uid) { result in
                            ...
                            case .success(let url):
                                print("Storage upload succeede. downloadURL:", url)
            ...
            .background(Color.white)
            .cornerRadius(15)
            .alert("Reset Password", isPresented: $showingPasswordReset, actions: {
                    TextField("Current Email", text: $currentEmail)
                    Button("Cancel", role: .cancel) { }
                    Button("Send") {
                        authModel.resetPassword(email: currentEmail, completion: { err in
                            if let error = err {
                                print("Error: ", error)
                            }
                        })
                    }
                    .disabled(currentEmail.isEmpty)
                }, message: {
...
//                    }
                    ...
                    .disabled(
                        ...
                        authModel.currentPassword != authModel.confirmPassword
                    )
                } message: {
...
//                .onChange(of: notificationsEnabled) { on in
//                    FirestoreService.shared.updateNotificationsEnabled(on)
//                }
...
//            notificationsEnabled = profile?.notificationsEnabled ?? true
    ...
    private func uploadPicked() {
        guard let img = selectedImage, let data = img.jpegData(compressionQuality: 0.5) else { return } //0.5 = mix between medium quality photo
        let uid = Auth.auth().currentUser!.uid
        StorageService.shared.uploadProfileImage(data, for: uid) { result in
            switch result {
            case .success(let url):
...
// MARK: ------------------Change User Details Sheets---------------
