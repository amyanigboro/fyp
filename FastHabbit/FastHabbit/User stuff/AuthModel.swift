...
//    }
    ...
    //send password reset email but check if the email belongs to a user first
    func resetPassword(email: String, completion: @escaping (String?) -> Void) {
    ...
    // actually perform the Firebase delete (once user confirms)
    func deleteAccountConfirmed(completion: ((Error?) -> Void)? = nil) {
        ...
        reauthenticate(password: currentPassword) { err in
            if let err = err {
                completion?(err)
                return
            }
            user.delete { deleterr in
                if let deleterr = deleterr {
                    return completion!(deleterr)
                }
                
                FirestoreService.shared.db
                    .collection("users")
                    .document(user.uid)
                    .delete { firestorerror in
                        if let firestorerror = firestorerror {
                            return completion!(firestorerror)
                        }
    ...
    func updateEmail(to newEmail: String, completion: @escaping (Error?) -> Void) {
        ...
        reauthenticate(password: currentPassword) { [weak self] err in
            ...
            Auth.auth().currentUser?.sendEmailVerification(beforeUpdatingEmail: self!.newEmail) { erro in
                completion(erro)
                FirestoreService.shared.db
                    .collection("users")
                    .document(uid)
